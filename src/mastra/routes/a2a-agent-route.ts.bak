...
import buildHolidayGreeting from "../tools/greeting-tool";
...
export const a2aAgentRoute = registerApiRoute("/a2a/agent/:agentId", {
  ...
  handler: async (c: MastraContext) => {
    try {
      ...
      // --- Case: Chat just opened (no user input yet) ---
      if (
        (!message && !messages) ||
        (Array.isArray(messages) && messages.length === 0)
      ) {
        const greetingText = await buildHolidayGreeting(agent, metadata);
